...
from sage.all import GF, Integers, PolynomialRing, ZZ


def p7_mod49_nim_relation_spec(d, q, exponent):
    """Encode the notebook identities, including prescribed selector inputs.

    Ordinary variables denote the oriented Hecke operators. The native
    verifier supplies the twist; coefficients here are not twisted again.
    Selector graph variables are projected raw digits, exactly as in the
    explicit Python replay. All divisions take place inside the source.
    """
    from verify_hecke_relations import relation_spec

    if exponent not in (2, 3):
        raise ValueError("the modulo-49 checks use exponent 2 or 3")
    r = (int(d) + 14*int(q)) % 42
    j = r % 6
    alpha, beta = SELECTOR_ROOTS[j]
    names = ("T3", "T29", "G") if exponent == 2 else (
        "T3", "T29", "Q", "G",
        "JU0", "JV0", "JU1", "JV1", "JU2", "JV2",
    )
    ring = PolynomialRing(Integers(7**exponent), names=names)
    variables = dict(zip(names, ring.gens()))
    T3, T29, G = (variables[name] for name in ("T3", "T29", "G"))
    ordinary = {"T3": 3, "T29": 29}
    divisions = {"G": (T29 - 2 - T3*(T3-alpha)*(T3-beta), 1)}
    if exponent == 2:
        return relation_spec([("G", (G**7-G)**3, 1)], ordinary, divisions)

    Q = variables["Q"]
    divisions["Q"] = (sum(c*T3**i for i, c in
                             enumerate(TANGENT_POLYNOMIALS[j])), 2)
    relations = [("Q", (Q**7-Q)**3, 1)]
    inputs = [ring.one()]
    bundle = p7_mod49_relation_polynomials()
    for index, centre in enumerate(CENTRES[j]):
        E6 = sum(c*T3**i for i, c in
                 enumerate(_lagrange(centre, CENTRES[j])))**6
        JU, JV = variables[f"JU{index}"], variables[f"JV{index}"]
        divisions[f"JU{index}"] = (E6*(T3-centre), 1)
        divisions[f"JV{index}"] = (E6*(T29-2), 1)
        for record in bundle["relations"]:
            if record["degree_residue_mod42"] != r or record["centre"] != centre:
                continue
            arguments = (Q, G, JU if record.get("raw_digit") == "U" else JV)
            # Lift the coefficients of the cubed F_7 polynomial, as in the
            # archived replay; do not attempt a ring map F_7 -> Z/343Z.
            terminal = ring.zero()
            polynomial = record["polynomial"]**bundle["fixed_relation_power"]
            for powers, coefficient in polynomial.dict().items():
                term = ring(ZZ(coefficient))
                for variable, power in zip(arguments, powers):
                    term *= variable**power
                terminal += term
            relations.append((record["label"], terminal, 1))
            inputs.append(E6)
    spec = relation_spec(relations, ordinary, divisions)
    for record, initial in zip(spec["relations"][1:], inputs[1:]):
...
class _Source:
    """Cyclic orders and scaling factors for the modulo-343 selector calculation.

    The dimension here counts cyclic generators and does not assert freeness.
    """
    order_exponents: tuple[int, ...]

    @property
    def dimension(self):
        """Return the number of cyclic coordinates, including the torsion coordinates."""
        return len(self.order_exponents)

    @property
    def scales(self):
        """Return the factors embedding each cyclic coordinate into the ambient modulus 343."""
        return np.asarray(
            [
                P ** (EXPONENT - order)
    ...
    @property
    def orders(self):
        """Return the actual orders of the cyclic coordinates."""
        return np.asarray(
...
def _order_exponent(modulus):
    """Return e for a cyclic order 7^e with 1<=e<=3; reject other orders."""
    modulus = int(modulus)
    exponent = 0
...
def _generators(source):
    """Return the scaled coordinate representatives of the cyclic generators."""
    return np.diag(source.scales).astype(np.int64)


def _encode_operator(source, operator):
    """Encode the operator images with the target-coordinate scaling used by this replay."""
...
def _add(*vectors):
    """Add encoded row elements and reduce modulo the working modulus."""
...
def _scale(vectors, scalar):
    """Multiply encoded row elements by the given scalar modulo the working modulus."""
    return (
...
def _apply(source, vectors, operator):
    """Apply an encoded endomorphism to row elements, respecting every cyclic order."""
...
def _divide(source, vectors, exponent):
    """Choose encoded outputs for 7^a y=u inside the same mixed module.

    Return None if the coordinate equations have no solution; no cancellation of torsion is used.
    """
    vectors = np.asarray(vectors, dtype=np.int64)
...
def _polynomial_operator(source, operator, coefficients):
    """Evaluate an ordinary polynomial from its ascending coefficient list on the encoded source."""
...
def _t29_difference(source, t3, t29, alpha, beta):
    """Form the numerator t_29-2-t_3(t_3-alpha)(t_3-beta) of the relation G_j."""
    identity = _generators(source)
...
def _multiply_polynomials(left, right):
    """Multiply ascending coefficient lists over F_7."""
...
def _lagrange(centre, centres):
    """Return the coefficients in 0,...,6 of the Lagrange selector for the specified centre."""
    numerator = [1]
    denominator = 1
...
def _multiply_terms(left, right):
    """Multiply sparse joint polynomial terms over F_7, collecting equal monomials."""
    result = {}
...
def _power_terms(terms, exponent):
    """Expand a positive power of the sparse polynomial over F_7 before lifting its coefficients."""
    answer = terms
...
def _solve_mod7(matrix, target):
    """Solve the displayed linear system over F_7 with free coordinates set to zero.

    Return None when the equations are inconsistent.
    """
...
def _divided_action(source, current, numerator, exponent, previous_division=0):
    """Choose the next division output, allowing a 7-torsion correction to an earlier divided output.

    The original input remains fixed. Failure of this restricted choice does not exclude all realizations.
    """
...
def _monomial(source, selected, numerators, divisions, powers):
    """Realize one expanded monomial, applying the rightmost division relation first.

    Return None if the chosen successive corrections cannot complete the chain.
    """
    current = selected
    previous_division = 0
...
def _terminal_ambiguity_suffices(source, value, terms, divisions):
    """Test whether the kernel of a final division can remove the terminal obstruction modulo 7.

    Only freedom at the actual last division of a monomial is available.
    """
...
def _relation_holds(source, selected, numerators, divisions, terms):
    """Test the selected polynomial relation by constructing its intermediate elements inside the source.

    Use allowed final-division freedom when needed; failure of this sufficient procedure is not nonexistence.
    """
