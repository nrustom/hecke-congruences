...
from mixed_endomorphisms import (image_coordinates, normalize_mixed_matrix,
...
def prepare_source_data(R, d, q, *, hecke_indices=None, ideal=None,
                        recursive=True, context=None, audit=False):
    """Prepare M_d or I M_d at orientation q over any Z/p^m.

    Recursive construction is the default: Section 3.10 below a_m+b_m,
    including all complement relations. With only (R,d,q), compute the
    source once and its Hecke actions lazily in those same coordinates.
    Supply ``hecke_indices`` to compute the actions recursively as well.
    Set ``recursive=False`` for the direct construction in any even degree.
    ...
    lazy = hecke_indices is None
    if lazy and (ideal is not None or context is not None):
        raise ValueError("supply hecke_indices for ideal sources or a shared context")
    if lazy and not recursive:
        P = (direct_manin_presentation(d,R,True) if sign is None else
             direct_signed_manin_presentation(d,R,sign,True))
        return {**common,"construction":"direct","presentation":P,
                "coordinates":chain_ring_manin_quotient_coordinates(P)}
    indices = () if lazy else tuple(ZZ(n) for n in hecke_indices)
    if not indices and not lazy:
        raise ValueError("supply at least one Hecke index")
    ctx = context or RecursiveContext(R,indices,retain_lifts=lazy)
    ...
    result = {**common,"construction":"recursive" if recursive else "direct",
            "presentation":None,
    ...
    if lazy:
        result["recursive_module"] = M
    return result


def _lazy_hecke_matrix(n, data, check_descent=False):
    """Compute and cache a Hecke action without recomputing source coordinates.

    Only the recursive quotient's representative monomials are acted on.
    The quotient retains its full torsion. This is a fresh-source path, not
    permission to supply missing operators for a compact archive.
    """
    n = ZZ(n)
    if n <= 0 or n % data["p"] == 0:
        raise ValueError("require a positive prime-to-p Hecke index")
    cache = data["archived_hecke_matrices"]
    if n in cache and (not check_descent or cache[n].get("descent_checked")):
        return cache[n]
    M, R = data["recursive_module"], data["coefficient_ring"]
    moduli = data["coordinates"]["coordinate_moduli"]
    support = (list(range(len(M.indices))) if check_descent else
               [j for j in range(len(M.indices)) if any(M.lifts.column(j))])
    images = matrix(R,len(support),len(moduli))
    for gamma in heilbronn_merel_matrices(n):
        images += symmetric_power_action(gamma,data["d"],R,
                        [M.indices[j] for j in support],M.indices)*M.reduction
    if check_descent:
        P = (direct_manin_presentation(data["d"],R) if data["sign"] is None else
             direct_signed_manin_presentation(data["d"],R,data["sign"]))
        if not mixed_matrix_is_zero(P["B_mod"]*images,moduli):
            raise ArithmeticError("Hecke action does not preserve Manin relations")
    T = matrix(R,normalize_mixed_matrix(M.lifts.matrix_from_columns(support)*images,moduli))
    if not mixed_endomorphism_is_well_defined(T,moduli):
        raise ArithmeticError("Hecke action violates cyclic coordinate orders")
    cache[n] = {"matrix":T,"normalized_matrix":T,"coordinate_moduli":moduli,
                "coordinate_exponents":M.exponents,"base_ring":R,
