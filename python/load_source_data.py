...
def _validate_degree_and_orientation(d, q):
    """Validate a nonnegative even coefficient degree and a nonnegative orientation; return integer values."""
    d = ZZ(d)
    q = ZZ(q)
...
def _sha256(path):
    """Return the SHA-256 digest of the archived bytes.

    This identifies the input file; it is not a verification of its equations.
    """
...
def _load_compact_source(R, d, q, path, arrays):
    ...
    recursive_maps = None
    if meta.get('relation_maps', False):
        if scope != 'manin' or meta.get('construction') != 'recursive':
            raise ValueError('recursive maps require a recursive Manin source')
        flag = arrays[prefix+'_recursive']
        if flag.shape != (1,) or flag.dtype.kind != 'u' or int(flag[0]) not in (0,1):
            raise ValueError('invalid recursive source flag')
        expected = d >= p**(m-1)*(p+1) and not (p == 2 and m == 1)
        if bool(flag[0]) != expected:
            raise ValueError('recursive flag disagrees with the Dickson degree range')
        recursive_maps = {'recursive': bool(flag[0])}
        if expected:
            for name in ('transfer_a', 'transfer_b', 'complement_images'):
                values = arrays[prefix+'_'+name]
                if values.ndim != 2 or values.shape[1] != rank or values.dtype.kind != 'u':
                    raise ValueError('invalid recursive map dimensions')
                if np.any(values >= int(p**m)):
                    raise ValueError('recursive map entries outside coefficient ring')
                recursive_maps[name] = values.tolist()
    return {**arithmetic,"d":ZZ(d),"q":ZZ(q),"sign":None if p==2 else (-1)**q,
            ...
            "archived_hecke_matrices":actions,"recursive_maps":recursive_maps}
