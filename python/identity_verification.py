...
from compute_source_data import prepare_source_data, _lazy_hecke_matrix


def _hecke_matrix_on_source(n, data, check_descent):
    """Return T_n on the prepared source, reusing its archived or lazily computed action.

    A descent check requires the ambient presentation; compact archives cannot supply it.
    """
    if "recursive_module" in data:
        return _lazy_hecke_matrix(n, data, check_descent)
...
def _vacuous_result(data, F, n, reason):
    """Record that the requested identity holds on the zero source, with the given reason."""
    return {
        "degree": data["d"],
