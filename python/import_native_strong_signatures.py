...
def sha256_file(path: Path) -> str:
    """Return the SHA-256 digest of a file, reading it in bounded blocks."""
...
def payload_sha1(value: dict[str, Any]) -> str:
    """Identify the serialized imported record using the native producer's SHA-1 convention."""
    encoded = json.dumps(value, ensure_ascii=False, separators=(",", ":"))
    return hashlib.sha1(encoded.encode("utf-8")).hexdigest().upper()


def atomic_json(path: Path, value: dict[str, Any]) -> None:
    """Publish the complete JSON record by an atomic rename; do not expose a partly written file."""
...
def polynomial(coefficients: Iterable[Any], variable: str = "x") -> str:
    """Decode the exact polynomial representation used in the imported certificate."""
    terms = []
...
def verify_source_envelope(
    ...
    """Check the manifest format, its recorded verification and its content digest.

    Return the manifest and its per-weight index, rejecting repeated weights.
    Arithmetic verification of individual records is separate.
    """
...
def convert_certificate(
    ...
    *,
    ...
    """Convert one native weight certificate to the repository format without changing the recorded signatures."""
...
def main() -> None:
    """Import compatible strong-signature records and publish their verified provenance and summaries."""
