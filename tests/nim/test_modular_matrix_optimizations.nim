...
proc filled(rows, columns: int; modulus: uint64; values: seq[uint64]): ModMatrix =
  ## Construct the row-major test matrix from an explicit list of residues.
...
suite "reusable FLINT buffers":
  test "prime-power solution extraction agrees with exhaustive enumeration":
    for setting in [(2'u64,2,4'u64),(3'u64,2,9'u64),(5'u64,1,5'u64)]:
      let (p,e,modulus)=setting
      for seed in 0..<80:
        let A=init_mod_matrix(2,2,modulus)
        let B=init_mod_matrix(2,1,modulus)
        A[0,0]=uint64(seed*17+4) mod modulus
        A[0,1]=uint64(seed*13+2) mod modulus
        A[1,0]=uint64(seed*7+1) mod modulus
        A[1,1]=uint64(seed*seed+3) mod modulus
        B[0,0]=uint64(seed*19+6) mod modulus
        B[1,0]=uint64(seed*11+5) mod modulus
        var exists=false
        for x in 0..<int(modulus):
          for y in 0..<int(modulus):
            if (A[0,0]*uint64(x)+A[0,1]*uint64(y)) mod modulus==B[0,0] and
                (A[1,0]*uint64(x)+A[1,1]*uint64(y)) mod modulus==B[1,0]:exists=true
        let X=solve_prime_power_system(A,B,p,e)
        check (not X.isNil)==exists
        if X!=nil:check A*X==B
    let A=init_mod_matrix(0,0,625)
    let B=init_mod_matrix(0,3,625)
    let X=solve_prime_power_system(A,B,5,4)
    check X.rows==0 and X.columns==3
  test "offset blocks match scalar arithmetic across moduli and SIMD tails":
    for modulus in [49'u64,343'u64,625'u64,2187'u64,65536'u64,1000003'u64]:
      for width in 0..19:
        let source=init_mod_matrix(3,width,modulus)
        let target=init_mod_matrix(6,width+5,modulus)
        for i in 0..<source.rows:
          for j in 0..<source.columns:source[i,j]=uint64(i*177+j*519+101) mod modulus
        for i in 0..<target.rows:
          for j in 0..<target.columns:target[i,j]=uint64(i*501+j*337+71) mod modulus
        let expected=target.copy()
        for i in 0..<source.rows:
          for j in 0..<source.columns:
            expected[i+2,j+3]=add_mod(expected[i+2,j+3],
              multiply_mod(modulus-1,source[i,j],modulus),modulus)
        target.add_scaled_block_from(source,2,3,modulus-1)
        check target==expected
    let same=identity_mod_matrix(2,625)
    expect ValueError:
      same.add_scaled_block_from(same,0,0,1)
