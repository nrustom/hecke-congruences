...
def test_fallback_does_not_fix_canonical_division():
    """Check that the fallback permits other intermediate choices after the first coordinatewise attempt fails."""
    R = Integers(9)
    S = PolynomialRing(R, names=('T', 'A', 'D'))
    T, A, D = S.gens()
...
def test_joint_mixed_circuit_against_exhaustive_search():
    """Compare joint division equations on a mixed source with exhaustive finite search."""
    R = Integers(9)
    S = PolynomialRing(R, names=('T', 'A', 'D'))
    T, A, D = S.gens()
