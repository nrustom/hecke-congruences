...
import std/[tables, algorithm, times, os, strutils]
import modular_matrix, modular_polynomial, manin_quotient, hecke_action
...
proc new_recursive_context*(p, m: int; hecke_indices: seq[int] = @[2];
                            ...
                            retain_lifts=true): RecursiveContext =
  ## Fix the working modulus and the full (not mod-p truncated) multipliers.
  if p<3 or p mod 2==0 or m<1: raise newException(ValueError, "require an odd prime and m>=1")
  var divisor=3
  ...
  let alpha=init_mod_polynomial(result.modulus)
  let beta=init_mod_polynomial(result.modulus)
  for j in 0..p: alpha[j*(p-1)]=1
  beta[p]=1; beta[1]=result.modulus-1
  result.a_polynomial=alpha.pow(result.t)
  result.b_polynomial=beta.pow(result.t)
...
proc module_cache_path(ctx:RecursiveContext; d,sign:int):string =
  ...
  if ctx.cache_directory.len==0 or d>=ctx.a or sign==0: return ""
  ctx.cache_directory / ("degree_" & $d & (if sign==1: "_plus.gz" else:"_minus.gz"))
...
proc load_module(ctx:RecursiveContext; d,sign:int):RecursiveModule =
  ...
  try: saved=read_checkpoint(path,ctx.module_cache_tag,d,sign,ctx.modulus)
    ...
    raise
  if saved.len!=3+ctx.hecke_indices.len or saved[0].rows!=1:
