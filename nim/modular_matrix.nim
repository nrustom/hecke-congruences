...
proc flint_matrix_transpose(target, source: ptr FlintNModMatrix)
  ...
  ## Write the transpose into an already initialized FLINT matrix of the required shape.

proc flint_matrix_inverse_prime(target, source: ptr FlintNModMatrix):cint
  {.importc: "nmod_mat_inv", dynlib: flint_library.}
  ## Call FLINT inversion over a prime field; this interface is not a composite-modulus solver.
...
#include <immintrin.h>
/* Update four reduced residues at a time for modulus <= 65536.
   Return the processed length; the caller handles the remaining entries. */
...
/* Compute scalar*source, optionally plus target, modulo the coefficient modulus.
   Inputs and scalar are reduced; use the AVX2 kernel only on a supporting CPU. */
...
/* Add a scaled row suffix, respecting the distinct source and target strides. */
...
/* Add a scaled rectangular source block at the given target offset. */
static void hecke_block_from(unsigned long *target, signed long ts,
    const unsigned long *source, signed long ss, signed long row,
    signed long column, signed long rows, signed long columns,
    unsigned long scalar, unsigned long modulus)
{
    for (signed long i=0; i<rows; ++i)
        hecke_update_row(target+(row+i)*ts+column,source+i*ss,
                         columns,scalar,modulus,1);
}

/* Multiply reduced residues for modulus <= 65536; the product fits in uint32_t. */
...
/* Reduce a 32-bit product using the precomputed reciprocal and one correction. */
...
/* Multiply machine-word residues without overflow, using a bounded small-modulus path. */
...
/* Clear logical entries without changing allocation or row stride. */
...
/* Add matrices modulo the common modulus without allocating a temporary matrix. */
...
/* Scale one logical row in place through the common modular update kernel. */
...
/* Add a scaled row in place. For distinct source and target rows this is
   an elementary operation preserving the row span. */
...
proc matrix_zero_word(
  ...
  ## Zero the logical matrix entries, respecting the physical row stride.
...
proc matrix_add_in_place_word(
  ...
  ## Add a source matrix in place modulo the common modulus, respecting both row strides.
...
proc scale_row_word(
  ...
  ## Multiply one row by a reduced scalar using the native modular row kernel.

proc add_scaled_row_word(
    entries: pointer; stride, columns, target, source: clong;
    scalar, modulus: culong
) {.importc: "hecke_nmod_add_scaled_row", nodecl.}
  ## Add a scalar multiple of one row to another modulo the coefficient modulus.
...
proc row_from_word(target:pointer; ts:clong; source:pointer; ss,row,source_row,
  ...
  ## Update a suffix of a target row from a source row, using the native small-modulus kernel when available.

proc block_from_word(target:pointer; ts:clong; source:pointer;
                     ss,row,column,rows,columns:clong; scalar,modulus:culong)
  {.importc:"hecke_block_from", nodecl.}
  ## Add a scaled source block at the given target offset, retaining row-stride conventions.
...
proc add_scaled_block_from*(target, source: ModMatrix; row,column:int;
                            scalar:uint64) =
  ## Add a rectangular source into a target block via the AVX2/scalar kernel.
  ## Aliasing is rejected to avoid overwriting overlapping input rows.
  if target.modulus!=source.modulus:
    raise newException(ValueError,"block update ring mismatch")
  if target==source:
    raise newException(ValueError,"block update requires distinct matrices")
  if row<0 or column<0 or row>target.rows or column>target.columns or
      source.rows>target.rows-row or source.columns>target.columns-column:
    raise newException(IndexDefect,"block update index out of bounds")
  if source.rows==0 or source.columns==0:return
  block_from_word(cast[pointer](target.raw.entries),target.raw.stride,
    cast[pointer](source.raw.entries),source.raw.stride,clong(row),clong(column),
    source.raw.row_count,source.raw.column_count,culong(scalar mod target.modulus),
    target.raw.modulus_data.n)
...
proc solve_prime_power_system*(original, right_hand_side:ModMatrix;
                               p:uint64; exponent:int):ModMatrix =
  ## Solve A X = B over Z/p^exponent using minimum-valuation pivots.
  ## Return nil if inconsistent. Unit row operations act on B as well;
  ## column permutations are undone after back substitution. No saturation.
  ## The caller must supply a prime p and replay the returned solution.
  if original.modulus!=right_hand_side.modulus or
      original.rows!=right_hand_side.rows or p<2 or exponent<1:
    raise newException(ValueError,"invalid prime-power linear system")
  let modulus=original.modulus
  var expected=1'u64
  for _ in 0..<exponent:
    if expected>high(uint64) div p:raise newException(ValueError,"modulus overflow")
    expected*=p
  if expected!=modulus:raise newException(ValueError,"wrong prime-power modulus")
  let A=original.copy()
  let B=right_hand_side.copy()
  var permutation=newSeq[int](A.columns)
  for j in 0..<A.columns:permutation[j]=j
  var lookup:seq[int]
  proc valuation(x:uint64):int =
    ## Return the p-adic valuation truncated at the working exponent, including zero.
    if x==0:return exponent
    var y=x
    while y mod p==0:y=y div p;inc result
  if modulus<=65536:
    lookup=newSeq[int](int(modulus))
    for x in 0..<lookup.len:lookup[x]=valuation(uint64(x))
  var rank=0
  for k in 0..<min(A.rows,A.columns):
    var best=exponent
    var pi=k
    var pj=k
    block search:
      for i in k..<A.rows:
        for j in k..<A.columns:
          let v=if lookup.len>0:lookup[int(A[i,j])] else:valuation(A[i,j])
          if v<best:
            best=v;pi=i;pj=j
            if best==0:break search
    if best==exponent:break
    if pi!=k:
      for j in 0..<A.columns:
        let v=A[k,j];A[k,j]=A[pi,j];A[pi,j]=v
      for j in 0..<B.columns:
        let v=B[k,j];B[k,j]=B[pi,j];B[pi,j]=v
    if pj!=k:
      for i in 0..<A.rows:
        let v=A[i,k];A[i,k]=A[i,pj];A[i,pj]=v
      swap(permutation[k],permutation[pj])
    var pivot=1'u64
    for _ in 0..<best:pivot*=p
    let inverse=inverse_mod(A[k,k] div pivot,modulus)
    A.scale_row_in_place(k,inverse)
    B.scale_row_in_place(k,inverse)
    for j in 0..<B.columns:
      if B[k,j] mod pivot!=0:return nil
    for i in k+1..<A.rows:
      let coefficient=subtract_mod(0,A[i,k] div pivot,modulus)
      if coefficient!=0:
        A.add_scaled_row_from(A,i,k,coefficient,k)
        B.add_scaled_row_from(B,i,k,coefficient)
    inc rank
  for i in rank..<B.rows:
    for j in 0..<B.columns:
      if B[i,j]!=0:return nil
  let X=init_mod_matrix(A.columns,B.columns,modulus)
  for k in countdown(rank-1,0):
    for j in 0..<B.columns:
      var rhs=B[k,j]
      for i in k+1..<A.columns:
        rhs=subtract_mod(rhs,multiply_mod(A[k,i],X[i,j],modulus),modulus)
      if rhs mod A[k,k]!=0:return nil
      X[k,j]=rhs div A[k,k]
  result=init_mod_matrix(A.columns,B.columns,modulus)
  for i in 0..<X.rows:
    for j in 0..<X.columns:result[permutation[i],j]=X[i,j]
...
proc read_checkpoint*(path,tag:string; d,sign:int; expected_modulus:uint64):seq[ModMatrix] =
  ...
  try:
    ...
    # Do not hide a useful header/read/CRC exception with a gzclose exception.
    discard gzclose(file)
    raise
  if gzclose(file)!=0:
    raise newException(IOError,"checkpoint close failed: " & path)
