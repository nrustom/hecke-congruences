...
For the other main cases, change the parameters to `(2,8)`, `(3,3)`, or
`(5,2)`. Higher exponents, including `(2,9)`, `(3,4)`, `(5,3)`, are allowed.
...
canonical embedding. The stored local-ring data determine the conjugate
pair of KRW classes. For nonrational packets, the TSV gives references to
...
weights. **Nonrational packets are not deduplicated across coefficient
fields**: that would require compatible embeddings into a common local
field. Their packet count is therefore not a count of distinct signatures.
All of the data are retained for that further comparison. This distinction
is important when exploring higher moduli.
...
detect accidental changes to saved data; they are not proof certificates
in Lean and are not substitutes for the exact arithmetic checks.
