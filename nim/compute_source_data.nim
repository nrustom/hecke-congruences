...
## By default, use recursive construction and write a compact version-3 archive:
...
## --direct selects direct construction; without --compact it preserves the
## legacy full-source replay format described above.
...
proc zlib_compress_bound(size: culong): culong
  ...
  ## Return zlib's upper bound on the destination size for compression.
proc zlib_compress(destination: ptr byte; size: ptr culong;
  ...
  ## Compress the source bytes into the supplied buffer and return the zlib status.
...
Optional:
  --recursive          Exact Section 3.10 construction below a_m+b_m (default).
  --direct             Disable recursion; use legacy replay unless --compact.
  ...
  Recursive whole-Manin archives also retain transfer/complement maps by default.
...
proc parse_command_line(): CommandLineOptions =
  ...
  proc following_value(option: string): string =
    ## Read the next command-line value, rejecting an option with no argument.
    index += 1
  ...
  while index < arguments.len:
    ...
    if argument == "--help" or argument == "-h":
      ...
      result.recursive=true
    elif argument == "--direct": result.recursive=false
...
proc compact_archive_arrays(options:CommandLineOptions):seq[NumpyUnsignedArray] =
  ...
  let context=new_recursive_context(p,m,options.hecke_indices,retain_lifts=options.replay_maps,
    retain_relation_maps=options.recursive and options.ideal_path.len==0)
  ...
  let metadata = %*{"prime":p,"exponent":m,"degree":d,
    ...
    "relation_maps":context.retain_relation_maps,
  ...
  var sources:Table[int,tuple[B,lifts,reduction:ModMatrix,
    actions:Table[int,ModMatrix],exponents:seq[int],relation_module:RecursiveModule]]
  for orientation_index,q in orientations:
    ...
    var relation_module:RecursiveModule
    if sources.hasKey(sign):
      ...
      relation_module=saved.relation_module
    ...
    sources[sign]=(B,lifts,reduction,actions,exponents,relation_module)
    if ideal!=nil:
    ...
    if context.retain_relation_maps:
      result.add(numpy_array(prefix & "_recursive",@[1],@[uint64(ord(relation_module.recursive))],1))
      if relation_module.recursive:
        for item in [("transfer_a",relation_module.transfer_a),
                     ("transfer_b",relation_module.transfer_b),
                     ("complement_images",relation_module.complement_images)]:
          let A=item[1]
          result.add(numpy_array(prefix & "_" & item[0],@[A.rows,A.columns],
            A.entries,choose_element_size(modulus-1)))
...
proc main() =
  ...
  try:
    ...
    let arrays = if options.recursive or options.compact:compact_archive_arrays(options)
