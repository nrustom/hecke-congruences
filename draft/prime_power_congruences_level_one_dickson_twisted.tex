...
For a presentation \(\mathfrak P\) over \(R=A/p^mA\), put \[ \mathfrak I:=\mathfrak D_{1,p^b}\mathfrak P. \] Then \[ \mathfrak I(M) = \{(x,\rho):\exists y\in\mathfrak P(M)(x),\ y=p^b\rho\}. \] Consequently, \[ \mathfrak P(M)\equiv0\pmod{p^b} \quad\Longleftrightarrow\quad \operatorname{dom}\mathfrak I(M)=M. \] Thus congruence conditions for presented relations pass through surjective module homomorphisms and hold on a finite direct sum whenever they hold on each summand.
...
\begin{proposition}[Propagation of presented relations]\label{prop:presented-relation-propagation} Put \(\delta_m:=\min(a_m,b_m)\), and let \(\mathfrak P\) be a presentation of a linear relation over \(\mathbb A_m\). For odd \(p\), suppose that \[ \operatorname{dom}\mathfrak P \bigl(\mathbb M_d^{\epsilon_q}(R_m)\bigr) = \mathbb M_d^{\epsilon_q}(R_m) \] for every even degree \[ \delta_m\leq d<a_m+b_m \] and every \(q\geq0\), with the actions specified above. Then the same assertion holds for every even \(d\geq\delta_m\) and every \(q\geq0\). 

For \(p=2\), if \[ \operatorname{dom}\mathfrak P \bigl(\mathbb M_d(R_m)\bigr) = \mathbb M_d(R_m) \] for every even \(\delta_m\leq d<a_m+b_m\), then it holds for every even \(d\geq\delta_m\). 

The same conclusions hold with the full-domain condition
replaced by
\[
\mathfrak P
\bigl(\mathbb M_d^{\epsilon_q}(R_m)\bigr)
\equiv0\pmod{p^b},
\]
or its unsplit analogue for \(p=2\), where \(0\leq b\leq m\).
\end{proposition}

\begin{proof}
We argue by strong induction on \(d\), simultaneously in
all orientations when \(p\) is odd. The assertion holds
in the stated initial range. If \(d\geq a_m+b_m\), both
\(d-a_m\) and \(d-b_m\) lie between \(\delta_m\) and \(d-1\).
By the induction hypothesis, the presented relation has
full domain on both source modules, hence on their direct
sum. Full domain passes through the surjective transfer
map, proving the assertion in degree \(d\).

For congruences, apply the same argument to the presentation
\[
\mathfrak D_{1,p^b}\mathfrak P.
\]
\end{proof}
...
and
\[
2^m\leq d<5\cdot2^{m-1}
\qquad(p=2).
\]
For odd \(p\), the specified module structures are periodic
in \(q\) with period \(p-1\), so only finitely many
orientations need to be checked.

As in Remark~\ref{rem:periodic-weight-propagation}, the
presentation may depend on degree residues and orientation.
The same induction applies provided that, at each transfer
step, the source conditions are those of the target
presentation evaluated on the two source modules with
their respective \(\mathbb A_m\)-actions.
Thus any family closed under the two degree transitions
propagates in the same way. One may likewise restrict
to degree residues closed under these transitions.

To obtain operator congruences, take orientation \(q=0\)
and use
\[
\mathbb M_d^+(R_m)
\twoheadrightarrow P_d^+/p^mP_d^+
\qquad(p\text{ odd}),
\]
or
\[
\mathbb M_d(R_m)
\twoheadrightarrow
\mathbb M_d^{\mathrm{tf}}/2^m\mathbb M_d^{\mathrm{tf}}
\qquad(p=2).
\]
Give the free lattices their \(\mathbb A\)-module structures
by letting \(t_n\) act as \(T_n\).
Proposition~\ref{prop:division-retained-precision}, applied
with \(A=\mathbb A\), then establishes integrality of the
divided operators and the resulting polynomial congruences
on the free lattice, under its stated hypotheses.

These conclusions restrict to \(L_d^+\). Indeed, the
numerator Hecke operators preserve \(L_d^+\), and saturation
implies that their integral divided operators preserve it
as well. For \(p=2\), these operators also commute with
\(\iota\), by cancellation on the torsion-free module,
and hence preserve \(P_d^+\). Finally, saturation gives
\[
L_d^+\cap p^bP_d^+=p^bL_d^+
\]
for odd \(p\), and
\[
L_d^+\cap 2^b\mathbb M_d^{\mathrm{tf}}=2^bL_d^+
\]
for \(p=2\).

Relations with prescribed inputs are included in the same
argument. For example,
\[
F(\mathcal Z_1,\ldots,\mathcal Z_s)\Gamma_S
\equiv0\pmod{p^b}
\]
is a presented relation, with the action of \(S\) twisted
along with the numerator operators.
...
Put \[ F_r(Z):=Z^2-\varepsilon_rZ. \] The computation is performed at precision \(256\).  For \(m=8\), the Dickson multipliers have degrees \[ a_8=256, \qquad b_8=384, \qquad \mbox{ with } a_8 + b_8 = 640. \] Hence the exact induction base in Corollary~\ref{cor:staged-witness-propagation-p2} consists, for every even \(0\leq r<128\), of the three degrees \[ d=r+j\,128, \qquad 2\leq j\leq4. \] Equivalently, these are the \(192\) even degrees \[ 256\leq d\leq638. \] 

For every such degree \(d\), the computation \cite[\nolinkurl{classification_mod_256.ipynb}]{RustomHeckeCongruences} verifies on \[ \mathbb M_d(\mathbf Z/256\mathbf Z) \] staged \(2^7\)-division witnesses of depth \(1\) for the pair \[ \bigl(X_r,F_r\bigr), \] and also verifies the ordinary relation \[ \bigl(T_5-c_{5,r}\bigr) \mathbb M_d(\mathbf Z/256\mathbf Z)=0. \] Since both \(a_8\) and \(b_8\) are divisible by \(128\), the quantities \(c_{3,r}\), \(c_{5,r}\), and \(\varepsilon_r\) are unchanged under the two transitions of Remark~\ref{rem:periodic-weight-propagation}.  Thus these families satisfy the compatibility required for propagation.

Corollary~\ref{cor:staged-witness-propagation-p2} and Theorem~\ref{thm:staged-division-propagation-p2} propagate the relations to every even degree \[ d\geq256. \] The remaining even degrees \[ 0\leq d<256 \] are verified directly. Therefore, for \[ r\equiv d\pmod{128}, \] the operator \[ Z_r:=\frac{T_3-c_{3,r}}{2^7} \] belongs to \[ \End_{\mathbf Z_2}\bigl(\mathbb M_d^{\mathrm{tf}}\bigr), \] preserves \(P_d^+\) and \(L_d^+\), and satisfies \begin{equation} \bigl(Z_r^2-\varepsilon_rZ_r\bigr) \mathbb M_d^{\mathrm{tf}} \subseteq2\mathbb M_d^{\mathrm{tf}}, \qquad \bigl(Z_r^2-\varepsilon_rZ_r\bigr)L_d^+ \subseteq2L_d^+. \label{eq:p2-divided-relation}\end{equation}
...
For every degree \[ 0\leq d\leq88, \qquad d\equiv r \pmod {18}, \] and both orientations \(q\in\{0,1\}\), the computation \cite[\nolinkurl{classification_mod_27.ipynb}]{RustomHeckeCongruences} certifies on \[ \mathbb M_d^{\epsilon_q}(\mathbf Z/27\mathbf Z) \] staged \(3\)-division witnesses of depth \(2\) for the pair \[ \bigl(T_2,F_r\bigr) \] in orientation \(q\), in the sense of Definition~\ref{def:staged-p-division-witness}. It also certifies \begin{equation} (T_7-t_{d\bmod18}) \mathbb M_d^{\epsilon_q}(\mathbf Z/27\mathbf Z)=0. \label{eq:p3-nonzero-source-relations} \end{equation}

By Theorem~\ref{thm:staged-division-propagation} and Theorem~\ref{thm:weight-propagation}, these relations propagate to every even coefficient degree \[ d \geq36, \qquad d \equiv r \pmod{18}. \] The remaining degrees \[ 0\leq d<36, \qquad d\equiv r \pmod{18}, \] are verified directly. Consequently, the relations hold in every even coefficient degree \[ d\equiv r \pmod{ 18}. \] For such a degree, we find that \[ Z_2:=\frac{T_2}{3}\in\End_{\Z_3}(P_d^+), \qquad Z_2L_d^+\subseteq L_d^+, \] and \[ \bigl(Z_2^2-c_r\bigr)P_d^+\subseteq9P_d^+. \] Similarly, \[ (T_7-t_r)P_d^+\subseteq27P_d^+. \] Consequently, \begin{equation} \bigl(Z_2^2-c_r\bigr)L_d^+ \subseteq9L_d^+, \qquad (T_7-t_r)L_d^+ \subseteq27L_d^+.  \label{eq:p3-nonzero-relations} \end{equation}
...
For every degree \[ 326\leq d\leq806, \qquad d\equiv2\pmod6, \] and both orientations \(q\in\{0,1\}\), the computation \cite[\nolinkurl{classification_mod_27.ipynb}]{RustomHeckeCongruences} verifies on \[ \mathbb M_d^{\epsilon_q}(\mathbf Z/243\mathbf Z) \] staged \(3^4\)-division witnesses of depth \(1\) for the pair \[ (G_2,Q) \] in orientation \(q\), in the sense of Definition~\ref{def:staged-p-division-witness}. Here \[ \chi_5^0(G_2)=T_2(T_2-63), \qquad \chi_5^1(G_2)=T_2(T_2+63) \pmod{243}. \] 

Corollary~\ref{cor:staged-witness-propagation} and Theorem~\ref{thm:staged-division-propagation} therefore give, for every even degree \[ d\geq324, \qquad d\equiv2\pmod6, \] an integral operator \begin{equation} H_2:=\frac{G_2}{81} =\frac{T_2(T_2-63)}{81} \in\End_{\mathbf Z_3}(P_d^+), \qquad H_2L_d^+\subseteq L_d^+. \label{eq:p3-zero-H2-integrality} \end{equation} It satisfies \begin{equation} Q(H_2)P_d^+\subseteq3P_d^+, \qquad Q(H_2)L_d^+\subseteq3L_d^+. \label{eq:p3-zero-H2-relation} \end{equation} The computation verifies \eqref{eq:p3-zero-H2-integrality} and \eqref{eq:p3-zero-H2-relation} directly in the remaining \(54\) degrees \[ 0\leq d<324, \qquad d\equiv2\pmod6. \] Consequently, both relations hold in every zero-branch degree.
...
For each even degree \(30\leq d\leq128\), the computation also verifies staged \(5\)-division witnesses of depth \(1\) for \[ \left(T_{19}-5\beta_d,(X^3+X)^2\right) \] on \(\mathbb M_d^+(\mathbf Z/25\mathbf Z)\) in orientations \(q=0,2\), and for \[ \left(T_{19}-5\beta_{d+10},(X^3+X)^2\right) \] on \(\mathbb M_d^-(\mathbf Z/25\mathbf Z)\) in orientations \(q=1,3\), where the subscripts of \(\beta\) are understood modulo \(20\). These four conditions are closed under the two transitions of Remark~\ref{rem:periodic-weight-propagation}. Indeed, the \(A_2\)-transition preserves the residue, the sign, and the orientation, while the \(B_2\)-transition sends \[ (d,q)\longmapsto(d-30,q+1) \] and reverses the sign. Since \[ d-30\equiv d+10\pmod{20}, \] the corresponding \(T_{19}-5\beta_r\) operator is unchanged. Hence the same strong-induction argument as in Corollary~\ref{cor:staged-witness-propagation} propagates these witnesses to every even degree \(d\geq30\). By Proposition~\ref{prop:staged-p-division}, we obtain, for \(r\equiv d\pmod{20}\), \[ \frac{T_{19}-5\beta_r}{5} \in\End_{\mathbf Z_5}(P_d^+). \] Consequently, \[ Z_{19}:=\frac{T_{19}}5\in\End_{\mathbf Z_5}(P_d^+), \qquad Z_{19}L_d^+\subseteq L_d^+, \] and \[ Q_r(Z_{19})^2P_d^+\subseteq5P_d^+. \] By saturation, \[ Q_r(Z_{19})^2L_d^+\subseteq5L_d^+. \] The remaining even degrees \(0\leq d<30\) are verified directly. Thus, in every even degree, \begin{equation} Q_r(Z_{19})^2P_d^+\subseteq5P_d^+, \qquad Q_r(Z_{19})^2L_d^+\subseteq5L_d^+. \label{eq:p5-divided-relation}
\end{equation}
...
We first treat \(Q_{r\bmod6}(T_3)\). The computation is performed at precision \(343=7^3\). The Dickson multipliers have degrees \[ a_3=2058,\qquad b_3=392, \qquad\mbox{with}\qquad a_3+b_3=2450,\] so the exact induction base consists of the even degrees \[ 392\leq d<2450. \] For every such \(d\) and every \(0\leq q<6\), the computation \cite[\nolinkurl{classification_mod_49.ipynb}]{RustomHeckeCongruences} verifies on \[ \mathbb M_d^{\epsilon_q}(\mathbf Z/343\mathbf Z), \qquad \epsilon_q=(-1)^q, \] staged \(49\)-division witnesses of depth \(1\), in orientation \(q\), for \[ \bigl(F_{(d+2q)\bmod6}(T_3),H\bigr). \] These six conditions are closed under both transitions of Remark~\ref{rem:periodic-weight-propagation}: the \(A_3\)-transition preserves \(d+2q\pmod6\), while the \(B_3\)-transition sends \[ (d,q)\longmapsto(d-392,q+1) \] and again preserves \(d+2q\pmod6\).

Now fix an even residue \(r\bmod42\). The preceding transition compatibility and the same strong-induction argument as in Corollary~\ref{cor:staged-witness-propagation} propagate these witnesses to every degree \[ d\geq392, \qquad d\equiv r\pmod{42}. \] Applying Proposition~\ref{prop:staged-p-division} gives \[ Q_{r\bmod6}(T_3)\in\End_{\mathbf Z_7}(P_d^+), \qquad Q_{r\bmod6}(T_3)L_d^+\subseteq L_d^+. \] Moreover, \begin{equation} H\bigl(Q_{r\bmod6}(T_3)\bigr)P_d^+ \subseteq7P_d^+, \qquad H\bigl(Q_{r\bmod6}(T_3)\bigr)L_d^+ \subseteq7L_d^+. \label{eq:p7-Q-relations} \end{equation} The remaining degrees \[ 0\leq d<392, \qquad d\equiv r\pmod{42}, \] are verified directly. Consequently, these conclusions hold in every even degree \[ d\equiv r\pmod{42}. \]

We next treat \(G_{r\bmod6}(T_3,T_{29})\).  This computation is performed at precision \(49\).  The Dickson multipliers have degrees \[ a_2=294,\qquad b_2=56, \qquad\mbox{with}\qquad a_2+b_2=350, \] so the exact induction base consists of the even degrees \[ 56\leq d<350. \] For every such \(d\) and every \(0\leq q<6\), the computation verifies on \[ \mathbb M_d^{\epsilon_q}(\mathbf Z/49\mathbf Z), \qquad \epsilon_q=(-1)^q, \] staged \(7\)-division witnesses of depth \(1\), in orientation \(q\), for the pair \[ \bigl( 7G_{(d+2q)\bmod6}(T_3,T_{29}),H \bigr). \] These six conditions are closed under both transitions of Remark~\ref{rem:periodic-weight-propagation}. Hence the same strong-induction argument as in Corollary~\ref{cor:staged-witness-propagation} propagates these witnesses to every degree \[ d\geq56, \qquad d\equiv r\pmod{42}. \] Applying Proposition~\ref{prop:staged-p-division} in the untwisted orientation gives \[ G_{r\bmod6}(T_3,T_{29}) \in\End_{\mathbf Z_7}(P_d^+), \qquad G_{r\bmod6}(T_3,T_{29})L_d^+\subseteq L_d^+. \] Moreover, \begin{equation} H\bigl(G_{r\bmod6}(T_3,T_{29})\bigr)P_d^+ \subseteq7P_d^+, \qquad H\bigl(G_{r\bmod6}(T_3,T_{29})\bigr)L_d^+ \subseteq7L_d^+. \label{eq:p7-G-relations} \end{equation} The remaining degrees \[ 0\leq d<56, \qquad d\equiv r\pmod{42}, \] are verified directly. Consequently, these conclusions hold in every even degree \[ d\equiv r\pmod{42}. \]
